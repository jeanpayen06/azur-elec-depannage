(function () {
  ...
  if (burger && nav) {
    ...
    function onMobileMenuQueryChange(event) {
      if (!event.matches) {
        setMenuOpen(false);
      }
    }
    ...
    if (mobileMenuQuery.addEventListener) {
      mobileMenuQuery.addEventListener('change', onMobileMenuQueryChange);
    } else {
      mobileMenuQuery.addListener(onMobileMenuQueryChange);
    }
  }
